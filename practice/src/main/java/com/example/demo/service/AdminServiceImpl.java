...
public class AdminServiceImpl implements AdminService {
    ...
    public void saveAdmin(Admin admin) {
        String encodedPassword = passwordEncoder.encode(admin.getPassword()); // パスワードをエンコードする
        admin.setPassword(encodedPassword); // エンコードしたパスワードをセットする
    ...
    public boolean authenticate(String email, String password) {
        ...
        if (admin.isPresent()) {
            boolean isPasswordMatch = passwordEncoder.matches(password, admin.get().getPassword()); // パスワードの照合
            if (isPasswordMatch) {
    ...
    public void registerAdmin(String email, String password, String firstName, String lastName) {
        ...
        admin.setPassword(password); // 平文のパスワードを保存
        admin.setFirstName(firstName);
        admin.setLastName(lastName);
        saveAdmin(admin); // パスワードを暗号化して保存
    }
